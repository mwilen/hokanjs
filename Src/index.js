var Interpolator = function(keys){
    ...
    props.onChange = function(cb){
      var returnProps = JSON.parse(JSON.stringify(props));
    ...
    Array.prototype.slice.call(document.querySelectorAll('*')).forEach((elem) => {
      if(elem.tagName && !elem.tagName.match(/(html|body|script|style|meta|title|head|link)/gi)){
        ...
        if(elem.innerText.match(stringRegExp)){
