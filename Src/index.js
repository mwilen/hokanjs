var Hokan = function(keys) {
    ...
    Array.prototype.slice.call(document.querySelectorAll("*")).forEach(elem => {
        if (
            elem.tagName &&
            !elem.tagName.match(/(html|body|script|style|meta|head|link)/gi)
        ) {
            let hasPlaceholderAttr = false;
            for (let i = 0; i < elem.attributes.length; i++) {
                let attr = elem.attributes[i];
                if (attr.value.match(stringRegExp)) {
                    attr.value.match(stringRegExp).forEach(match => {
            ...
            if (!hasPlaceholderAttr && elem.innerText.match(stringRegExp)) {
                ...
                elem.innerText.match(stringRegExp).forEach(match => {
                    placeholders.push(match);
                });
            }
        }
            ...
            placeholders.forEach(p => {
                ...
                if (element.hasPlaceholderAttribute) {
                    for (let i = 0; i < element.clone.attributes.length; i++) {
                        element.clone.attributes[
                            i
                        ].value = element.clone.attributes[i].value.replace(
                            p,
                            props[key]
                        );
                    }
                }
                element.clone.innerText = element.clone.innerText.replace(
                    p,
                    props[key]
                );
            });
